...
def process_data(df):
    ...
    try:
        ...
        # Limit concentration values to 50
        df['Value'] = df['Value'].clip(upper=50)
...
def plot_distributions(data, site_name):
    ...
    try:
        # Make figure taller to accommodate larger boxes
        ...
        # Create box plot with custom colors and larger boxes
        ...
        # Color boxes based on compound category with higher opacity
        ...
        plt.title(f'Distribution of Air Quality Compounds\nat {site_name}\n(Colored by Refinery Association)\n(Concentrations limited to 50 µg/m³)', 
                 fontsize=16, pad=20)
        plt.xlabel('Compound', fontsize=14)
        plt.ylabel('Concentration (limited to 50)', fontsize=14)
        plt.xticks(rotation=45, ha='right', fontsize=10)
        plt.yticks(fontsize=10)
        
        # Add legend with larger patches
        legend_elements = [plt.Rectangle((0,0), 1, 1, 
                                      facecolor=color, 
                                      alpha=0.8,  # Match box opacity
                                      linewidth=2,  # Match box line width
                                      label=category)
                         for category, color in CATEGORY_COLORS.items()]
        plt.legend(handles=legend_elements, 
                  loc='upper right', 
                  title='Compound Categories',
                  title_fontsize=12,  # Increase legend title size
                  fontsize=10)  # Increase legend text size
        
        # Adjust layout to prevent label cutoff
        plt.tight_layout()
        plt.savefig(os.path.join('plots', f'{site_name}_distributions.png'), 
                    bbox_inches='tight', 
...
def plot_hourly_patterns(data, site_name):
    ...
    try:
        ...
        # Plot hourly patterns with category-based colors
        ...
        plt.ylabel('Average Concentration (limited to 200)', fontsize=14)
        ...
        plt.tight_layout()
        plt.savefig(os.path.join('plots', f'{site_name}_hourly_patterns.png'), bbox_inches='tight')
        plt.close()
    except Exception as e:
...
def plot_weekly_patterns(data, site_name):
    ...
    try:
        ...
        plt.title(f'Weekly Patterns of Air Quality Compounds\nat {site_name}', 
                 fontsize=16, pad=20)
        ...
        plt.tight_layout()
        plt.savefig(os.path.join('plots', f'{site_name}_weekly_patterns.png'), bbox_inches='tight')
        plt.close()
    except Exception as e:
...
def plot_monthly_trends(data, site_name):
    ...
    try:
        ...
        plt.title(f'Monthly Trends of Air Quality Compounds\nat {site_name}', 
                 fontsize=16, pad=20)
        ...
        plt.tight_layout()
        plt.savefig(os.path.join('plots', f'{site_name}_monthly_trends.png'), bbox_inches='tight')
        plt.close()
    except Exception as e:
...
def plot_wind_rose(data, site_name):
    """Create wind rose plot showing wind direction and speed patterns."""
    try:
        # Check if wind data columns exist
        if 'Wind Speed' not in data.columns or 'Wind Direction' not in data.columns:
            print(f"Wind data not available for {site_name}, skipping wind rose plot")
            return
            
        plt.figure(figsize=(12, 8))
        
        # Create wind direction bins
        wind_dirs = np.linspace(0, 360, 16, endpoint=False)
        wind_speeds = np.linspace(0, data['Wind Speed'].max(), 5)
        
        # Calculate frequency in each bin
        hist, _, _ = np.histogram2d(
            data['Wind Direction'],
            data['Wind Speed'],
            bins=[wind_dirs, wind_speeds]
        )
        ...
        # Plot wind rose
        for i, speed in enumerate(wind_speeds[:-1]):
            ax.bar(
                np.radians(wind_dirs[:-1]),
                hist[:, i],
                width=np.radians(360/16),
                bottom=np.sum(hist[:, :i], axis=1),
                label=f'{speed:.1f}-{wind_speeds[i+1]:.1f} m/s'
            )
        
        plt.title(f'Wind Rose - {site_name}', pad=20)
        plt.legend(title='Wind Speed', bbox_to_anchor=(1.1, 1))
        plt.tight_layout()
        plt.savefig(os.path.join('plots', f'{site_name}_wind_rose.png'), bbox_inches='tight')
        plt.close()
        
    except Exception as e:
        print(f"Error creating wind rose plot for {site_name}: {str(e)}")
...
try:
    ...
    for site in sites:
        ...
        print("- Generating wind rose plot...")
        plot_wind_rose(site_data, site)
